<?php

namespace app\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\Slicebot;

/**
 * SlicebotSearch represents the model behind the search form of `app\models\Slicebot`.
 */
class SlicebotSearch extends Slicebot
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['slicebot_id'], 'integer'],
            [['pid', 'server', 'cuenta', 'estado', 'activo', 'fechaCreado', 'fecha', 'duracion'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Slicebot::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);
        $dataProvider->setSort([
            'defaultOrder' => [
            'duracion' => SORT_ASC
        ]
    ]);
        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'slicebot_id' => $this->slicebot_id,
            'fechaCreado' => $this->fechaCreado,
            'fecha' => $this->fecha,
        ]);

        $query->andWhere('estado= :estado', [':estado' => 'activo']);

        $query->andFilterWhere(['like', 'pid', $this->pid])
            ->andFilterWhere(['like', 'server', $this->server])
            ->andFilterWhere(['like', 'cuenta', $this->cuenta])
            ->andFilterWhere(['like', 'estado', $this->estado])
            ->andFilterWhere(['like', 'activo', $this->activo])
            ->andFilterWhere(['like', 'duracion', $this->duracion]);

        return $dataProvider;
    }
}
